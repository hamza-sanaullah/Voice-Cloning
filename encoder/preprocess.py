...
def preprocess_librispeech(datasets_root: Path, out_dir: Path, skip_existing=False):
    """
    Preprocesses the LibriSpeech dataset, specifically the 'train-clean-100' subset, 
    converting audio files into mel spectrograms for encoder training.
    """
    # Define the subset you have
    librispeech_subset = "train-clean-100"
    
    # Initialize the preprocessing for the specified dataset
    dataset_root = datasets_root.joinpath("LibriSpeech", librispeech_subset)
    if not dataset_root.exists():
        print(f"Couldn't find {dataset_root}, skipping this dataset.")
        return

    print(f"Processing dataset: {librispeech_subset}")

    # Create output directory if it doesn't exist
    out_dir = out_dir.joinpath(librispeech_subset)
    out_dir.mkdir(parents=True, exist_ok=True)

    # Preprocess all speaker directories within the dataset
    speaker_dirs = list(dataset_root.glob("*"))
    for speaker_dir in speaker_dirs:
        print(f"Processing speaker: {speaker_dir.name}")
        for book_dir in speaker_dir.glob("*"):
            for audio_path in book_dir.glob("*.flac"):
                # Define the output path for the mel spectrogram
                out_path = out_dir.joinpath(audio_path.stem + ".npy")
                ...
                try:
                    ...
                    wav = audio.preprocess_wav(audio_path)
                    mel_spectrogram = audio.wav_to_mel_spectrogram(wav)
                    np.save(out_path, mel_spectrogram)
